...
use crate::{
    non_cortex::{
        ...
        error::{NonCortexError, arithmetic_error, invalid_request},
        types::{
            AdmissionDisposition, AdmissionReport, AdmissionReportItem, AdmissionWhy,
            AffordabilitySnapshot, AttributionRecord, IntentAttempt, NonCortexState,
            ReservationDelta,
        },
    },
...
impl CostAdmissionPolicy {
    fn reserve_amount_micro(&self, required_survival_micro: i64) -> Result<i64, NonCortexError> {
...
impl AdmissionResolver {
    ...
    pub fn admit_attempts(
        &self,
        state: &mut NonCortexState,
        cycle_id: u64,
        attempts: Vec<IntentAttempt>,
    ) -> Result<(AdmissionReport, AdmittedActionBatch), NonCortexError> {
        ...
        for attempt in sorted_attempts {
            ...
            let snapshot = self
                .cost_policy
                .affordability_snapshot(state.ledger.available_survival_micro(), &estimated_cost);
            ...
            let degraded = self.find_degraded_candidate(state, profile, &attempt)?;
    ...
    ) -> Result<(AdmittedAction, i64, AdmissionReportItem), NonCortexError> {
        let reserve_survival_micro = self
            .cost_policy
            .reserve_amount_micro(estimated_cost.survival_micro.max(0))?;

        let reserve_entry_id = state.ledger.reserve(
            cycle_id,
    ...
    fn find_degraded_candidate<'a>(
        ...
        attempt: &IntentAttempt,
    ) -> Result<Option<(&'a DegradationProfile, CostVector)>, NonCortexError> {
