...
pub struct DegradationProfile {
    ...
    pub capability_handle_override: Option<String>,
}
...
pub struct AffordanceRegistry {
    profiles_by_key: BTreeMap<String, AffordanceProfile>,
}
...
impl AffordanceRegistry {
    pub fn new(profiles: impl IntoIterator<Item = AffordanceProfile>) -> Self {
        ...
        for profile in profiles {
            profiles_by_key.insert(profile.affordance_key.clone(), profile);
        }
        Self { profiles_by_key }
    }

    pub fn resolve(&self, affordance_key: &str) -> Option<&AffordanceProfile> {
        self.profiles_by_key.get(affordance_key)
    }
        ...
        Self {
            ...
            degradation_profiles: vec![
                ...
                DegradationProfile {
                    ...
                    capability_handle_override: Some("cap.core.minimal".to_string()),
                },
            ],
        }
    }
