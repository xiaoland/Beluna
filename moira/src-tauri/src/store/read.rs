...
use crate::model::{EventRecord, RunSummary, TickDetail, TickSummary};
        ...
        let rows = stmt
            .query_map([], |row| {
                Ok(RunSummary {
                    ...
                    latest_tick: row.get::<_, Option<i64>>(6)?.map(|value| value.max(0) as u64),
                })
            })
        ...
        let rows = stmt
            .query_map(params![run_id, tick as i64], |row| {
                Ok(EventRecord {
                    ...
                    run_id: row.get(4)?,
                    tick: row.get::<_, Option<i64>>(5)?.map(|value| value.max(0) as u64),
                    target: row.get(6)?,
                    family: row.get(7)?,
                    subsystem: row.get(8)?,
...
fn infer_subsystem(event: &EventRecord) -> Option<&str> {
    event
        .subsystem
        .as_deref()
        .or_else(|| event.family.as_deref().and_then(|family| family.split('.').next()))
        .or_else(|| event.target.as_deref().and_then(|target| target.split('.').next()))
}
...
fn collect_rows<T>(rows: duckdb::MappedRows<'_, impl FnMut(&duckdb::Row<'_>) -> duckdb::Result<T>>) -> Result<Vec<T>, String> {
    rows.collect::<Result<Vec<_>, _>>()
        .map_err(|err| format!("failed to collect Moira rows: {err}"))
}
