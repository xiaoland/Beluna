...
const activeTab = ref<DetailTab>('cortex')
...
const selectionHint = computed(() => {
  if (!selectedTickDetail.value) {
    return 'Select a wake session and tick to inspect its cognition narrative.'
  }

  return `Tick ${selectedTickDetail.value.tick} from wake ${selectedTickDetail.value.runId} · ${selectedTickDetail.value.rawEvents.length} raw events · updated ${formatWhen(
...
.selection-hint {
  ...
  background: rgba(254, 248, 239, 0.76);
