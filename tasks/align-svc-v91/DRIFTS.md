...
> Status note: this file records the pre-alignment diagnosis snapshot.
> Current disposition after the Beluna alignment is tracked in [`RESULT.md`](./RESULT.md).
> Path strings inside this note were mechanically normalized during the `docs/task` -> `tasks/` migration, so treat it as a historical drift register rather than a byte-accurate before-image.
...
- Beluna's current framework routes volatile work to `tasks` and treats that path as a stable repo convention.
- Why this matters:
...
- Evidence:
  ...
  - `tasks/README.md`: lines 1-3, 39-45
...
- Evidence:
  ...
  - `tasks/README.md`: lines 10-20, 31-37, 47
...
1. Replace every `tasks/` reference with `tasks`.
