...
3. Auth is handled by adapter; auth payload carries registration info; adapter calls Spine `new_body_endpoint`.
4. Body endpoint may send unplug request; adapter calls Spine `remove_body_endpoint`.
5. Only capability changes flow through sense queue.
6. No capability-singleton enforcement in catalog; Act includes fully-qualified endpoint name and capability routing is completed within body endpoint.
7. No compatibility mode required.
...
- `new_body_endpoint(authenticated_registration) -> BodyEndpointHandle`
  - Input includes semantic base name declared by endpoint + adapter/session identity.
...
3. Transform auth-carried registration payload into Spine `new_body_endpoint` request.
4. Accept `sense` method for domain senses and capability change intents only.
5. Accept unplug request from endpoint, map to Spine `remove_body_endpoint`.
6. Outbound adapter->endpoint uses only `act` method frames.
...
### WS7 — Boot order cleanup (P2)

- Start Spine runtime/lifecycle before starting in-core body endpoints.
- Start in-core body endpoints via inline adapter after Spine starts.
- Start Stem after endpoint startup stage.
...
- Auth -> `new_body_endpoint` transform contract.
...
- Endpoint registration path is: endpoint `auth` -> adapter auth -> Spine `new_body_endpoint` -> fully-qualified name assigned.
